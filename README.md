...
```bash
conda create -n dbx_mlops_poc python=3.9
conda activate dbx_mlops_poc
```
...
```
This command will create your personal repository under `/Repos/<username>/dbx_mlops_poc`.
...
- To trigger the release pipeline, get the current version from the `dbx_mlops_poc/__init__.py` file and tag the current code version:
```
